...
### Phase 3 — Habit Engine (validated 2026-07-11, see docs/validation-report.md)

- **FR-017 (P0):** "Today is..." schedule suggestion — the plan carries a `weeklySchedule` map (KB routine: Mon Strength A · Tue Mobility · Wed Strength B · Thu Mobility · Fri Strength A · Sat Mobility/stretch · Sun recovery); the Today view leads with today's suggested session, one tap to start. All other sessions remain reachable in one tap (suggestion, never enforcement). *AC: on a Wednesday, Strength B is the lead card; picking any other session takes exactly one tap; a rest day shows "Recovery day" with sessions still reachable.*
- **FR-018 (P0):** Forgiveness nudge — a status line on the Today view computed from the existing log: "N strength sessions this week" against a target of 2+, plus the KB rule surfaced only when relevant ("last strength session was X days ago — never skip twice in a row"). Weekly reset; NEVER an unbroken-streak counter; copy must read as help, not scolding. *AC: one missed week does not change how the following week is displayed; nudge line appears only after ≥3 days without a strength session.*
- **FR-019 (P0):** Warm-up-first flow — opening a strength session presents the 8 warm-up cards first (5–7 min, ending with the KB completion checklist), then flows into the strength cards; a visible "skip warm-up" link keeps autonomy. Warm-up completion is recorded in the LogEntry. *AC: default path goes through warm-up; skip takes one tap; the log distinguishes warmed-up sessions.*
- **FR-020 (P1, deferred to ~week 4):** Monthly mobility self-assessment — the KB's 6 tracked movements as a 2-minute form (per movement: better / same / worse + optional note), stored in `localStorage` (`mgt.assessments.v1`), shown month-over-month; Today view prompts when ≥28 days since the last assessment. Included in export/import. *AC: two saved assessments render side by side; prompt appears only when due.*
...
8. **Text-free images (2026-07-06).** The generated image contains no text — figures, arrows, colour encoding, muscle glow and a red-X/green-check mistake inset only. All wording renders as HTML from `exercises.js` in the app and print sheet (image models garble text; HTML never does). The exercise card component = AI image + HTML text block (name, dose, Works, mistake/correct, cues).
